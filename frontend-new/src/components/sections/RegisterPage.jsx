...
export function RegisterPage({ setPage, onRegistrationComplete }) {
  ...
  const setRegistrationType = (value) => { setType(value); setDraft(null); setPaymentUploadToken(''); setErrors({}); };
  const updateTeamSize = (size) => {
    ...
    setDraft(null); setPaymentUploadToken('');
  };
  const updateMember = (index, key, value) => { setTeamMembers((current) => current.map((member, memberIndex) => memberIndex === index ? { ...member, [key]: value } : member)); setDraft(null); setPaymentUploadToken(''); };
  const selectScreenshot = (file) => { if (file) edit('payment', file); setIsDraggingScreenshot(false); };
  const toggle = (slug) => {
  ...
  const generateQr = async () => {
    ...
    try { const response = await api('/registrations/payment-quote', { method: 'POST', body: JSON.stringify(buildPayload()) }); setDraft(response.data); }
  ...
  const submitPayment = async () => {
    ...
    try {
      ...
      if (!token) { const upload = new FormData(); upload.append('screenshot', form.payment); const uploaded = await apiUpload('/registrations/payment-screenshot', upload); token = uploaded.data.paymentUploadToken; setPaymentUploadToken(token); }
      const response = await api('/registrations', { method: 'POST', body: JSON.stringify({ ...buildPayload(), paymentUploadToken: token }) });
      setRegistrationId(response.data.registrationId); setDone(true); onRegistrationComplete?.();
    }
    catch (error) { setErrors({ submit: error.message }); } finally { setSubmitting(false); }
  };
  ...
  if (done) return <main className="confirmation"><div className="success-mark"><Check size={48} /></div><p className="eyebrow">PAYMENT SUBMITTED</p><h1>Registration<br /><span>under verification.</span></h1><p>We received your payment screenshot. Our team will verify it and contact you by email.</p><div className="confirmation-card"><span>Registration ID</span><strong>{registrationId}</strong><span>{type} · ₹{fee}</span></div><div className="confirmation-next-steps"><p>Check your inbox and spam folder for the verification email.</p><a className="whatsapp-community" href={whatsappCommunityUrl} target="_blank" rel="noopener noreferrer"><MessageCircle size={18} /> Join our WhatsApp community for future updates</a></div><Button className="confirmation-home" onClick={() => setPage('home')}>Back to home</Button></main>;
